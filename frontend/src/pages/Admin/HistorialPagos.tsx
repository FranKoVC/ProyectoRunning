...
const HistorialPagos = () => {
  ...
  useEffect(() => {
    ...
    const clientesUnicos = Array.from(
      new Set(datosPagos.map((pago) => pago.clienteId))
    ).map((clienteId) => {
  ...
  useEffect(() => {
    ...
    if (filtroCliente) {
      filtrados = filtrados.filter(
        (pago) => pago.clienteId === filtroCliente.value
      );
    }
    ...
    if (filtroEstado) {
      filtrados = filtrados.filter(
        (pago) => pago.estado === filtroEstado.value
      );
    }
  ...
  const generarPDF = () => {
    ...
    autoTable(doc, {
      head: [
        ["Cliente", "Membresía", "Monto", "Fecha", "Método de Pago", "Estado"],
      ],
  ...
  const validarPago = () => {
    if (pagoSeleccionado) {
      ...
      setPagos((prevPagos) =>
        prevPagos.map((p) =>
          p.id === pagoSeleccionado.id ? { ...p, estado: "Validado" } : p
        )
      );
      cerrarModal();
    }
  ...
  const rechazarPago = () => {
    if (pagoSeleccionado) {
      ...
      setPagos((prevPagos) =>
        prevPagos.map((p) =>
          p.id === pagoSeleccionado.id ? { ...p, estado: "Rechazado" } : p
        )
      );
      cerrarModal();
    }
              ...
              pagosFiltrados.map((pago, index) => (
                ...
                >
                  ...
                  <td className="p-3 text-center">
                    <span
                      className={`py-1 px-2 rounded-full text-xs font-medium ${getEstadoColor(
                        pago.estado
                      )}`}
                    >
                      {pago.estado}
          ...
          {pagoSeleccionado && pagoSeleccionado.estado === "Pendiente" && (
            <div>
              <div className="flex justify-end space-x-4">
                <button
                  onClick={() => setMostrarCajaTexto(true)}
